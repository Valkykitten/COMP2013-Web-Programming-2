...
function realtorFees(listing) {
  const cleanPrice = listing.price.replace("$", "").replace(",", "") // takes the $ and , out of the price string so it can turned into a number.
  const priceValue = parseFloat(cleanPrice)
  const realtorFee = priceValue * (priceValue <= 450000 ? .025 : .020)
  return (`the realtor fee will be $${realtorFee}`)
}
/**
...
const listingAscendingly = listings
  .slice()
  for (let ii = 0; ii < listingAscendingly.length; ii++ ) {
    const cleanPrice = listingAscendingly[ii].price.replace("$", "").replace(",", "")
    listingAscendingly[ii].price = parseFloat(cleanPrice)
  }
  console.log(listingAscendingly.sort((a, b) => a.price - b.price))
 ...
 */
//WRITE YOUR CODE BELOW
const newBuiltListings = listings.filter(listing => listing.propertySummary.builtIn.replace(/\D/g, "") >= 1990)

//this code goes through listings and filters all the listings by looking through the built in date and removes any that are older than 1990
